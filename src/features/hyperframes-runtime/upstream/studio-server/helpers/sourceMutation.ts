...
import { isAllowedHtmlAttribute, isSafeAttributeValue } from "./htmlAttrSafety.js";
import { ensureHfIds } from "../../parsers/hfIds.js";
import { readClipTiming, writeClipTiming } from "../../core/compositionContract.js";
import { scanCssRules } from "../../lint/cssRuleScanner.js";
...
function duplicateCssRulesForId(document: Document, originalId: string, newId: string): void {
  ...
  for (const styleEl of document.querySelectorAll("style")) {
    ...
    for (const rule of scanCssRules(css)) {
      if (!rule.selector.includes(idToken)) continue;
      const newSelector = duplicateIdSelector(rule.selector, originalId, newId);
      if (newSelector === rule.selector) continue;
      clones.push(`${newSelector} {${rule.body}}`);
    }
    if (clones.length > 0) {
      styleEl.textContent = `${css.trimEnd()}\n${clones.join("\n")}`;
    }
  }
}
...
function duplicateIdSelector(selector: string, originalId: string, newId: string): string {
  const idPattern = new RegExp(`#${escapeRegExp(originalId)}(?![-_a-zA-Z0-9])`, "g");
  return selector
    .split(",")
    .map((part) => part.replace(idPattern, `#${newId}`))
    .join(",");
}
