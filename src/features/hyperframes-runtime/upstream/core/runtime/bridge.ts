...
type RuntimePreviewWindow = Window & {
  __HF_PREVIEW_SESSION__?: RuntimePreviewSession;
};

function readRuntimePreviewSession(): RuntimePreviewSession | null {
  const session = (window as RuntimePreviewWindow).__HF_PREVIEW_SESSION__;
  if (!session || typeof session !== "object") return null;
  return session;
}

function withRuntimePreviewSession<T extends Record<string, unknown>>(message: T): T {
  const session = readRuntimePreviewSession();
  if (!session?.nonce) return message;
  return {
    ...message,
    projectId: typeof session.projectId === "string" ? session.projectId : undefined,
    compositionPath: typeof session.compositionPath === "string" ? session.compositionPath : undefined,
    nonce: session.nonce,
    sandboxToken: typeof session.sandboxToken === "string" ? session.sandboxToken : undefined,
  };
}
...
export function postRuntimeMessage(payload: RuntimeOutboundMessage): void {
  try {
    window.parent.postMessage(
      withRuntimePreviewSession({ ...payload, ...runtimeProtocolMetadata(runtimeProtocolFps) }),
      "*",
    );
  } catch (err) {
...
function rejectWrongRuntimeSession(data: BridgeControlData): boolean {
  const session = readRuntimePreviewSession();
  if (!session?.nonce) return false;
  if (data.nonce !== session.nonce) return true;
  if (typeof session.projectId === "string" && data.projectId !== session.projectId) return true;
  if (typeof session.compositionPath === "string" && data.compositionPath !== session.compositionPath) {
    return true;
  }
  if (typeof session.sandboxToken === "string" && data.sandboxToken !== session.sandboxToken) {
    return true;
  }
  return false;
}
...
export function installRuntimeControlBridge(deps: BridgeDeps): (event: MessageEvent) => void {
  const handler = (event: MessageEvent) => {
    ...
    if (rejectWrongRuntimeSession(data)) return;
