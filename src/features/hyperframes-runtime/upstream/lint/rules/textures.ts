...
import { scanCssRules, type ScannedCssRule } from "../cssRuleScanner";
...
function collectTextureCss(styles: LintContext["styles"]): {
  ...
  for (const style of styles) {
    const styleRules = scanCssRules(style.content);
    rules.push(...styleRules);

    // fallow-ignore-next-line complexity
    for (const rule of styleRules) {
      const selectors = rule.selectors;
      const hasMaskImage = rule.declarations.some(
        (declaration) =>
          declaration.property === "mask-image" ||
          declaration.property === "-webkit-mask-image",
      );
  ...
  for (const rule of rules) {
    // fallow-ignore-next-line complexity
    const selectors = rule.selectors;
    const hasDropShadow = rule.declarations.some(
      (declaration) =>
        declaration.property === "filter" && /\bdrop-shadow\s*\(/i.test(declaration.value),
    );
