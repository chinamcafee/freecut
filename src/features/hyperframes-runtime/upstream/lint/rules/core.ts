...
export const coreRules: Array<(ctx: LintContext) => HyperframeLintFinding[]> = [
  ...
  ({ styles, rootCompositionId, rootTag }) => {
    ...
    for (const style of styles) {
      for (const rule of scanCssRules(style.content)) {
  ...
  ({ tags, styles }) => {
    ...
    for (const style of styles) {
      for (const rule of scanCssRules(style.content)) {
        const disablesPointerEvents = rule.declarations.some(
          (declaration) =>
            declaration.property === "pointer-events" &&
            declaration.value.trim().toLowerCase() === "none",
        );
        if (!disablesPointerEvents) continue;
