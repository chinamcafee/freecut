...
import { inspectRuntimeProtocol } from "../core/runtime/protocol.js";

type SceneRecord = { id: string; start: number; duration: number };
export interface RuntimeMessageSessionValidationOptions {
  projectId?: string;
...
function matchesSession(
  ...
  event: MessageEvent,
  options: RuntimeMessageSessionValidationOptions | undefined,
): boolean {
  if (!expectedOriginMatches(event.origin, options?.expectedOrigin)) return false;
  if (!options?.sessionNonce) return true;
  if (data["nonce"] !== options.sessionNonce) return false;
  if (options.projectId && data["projectId"] !== options.projectId) return false;
  if (options.compositionPath && data["compositionPath"] !== options.compositionPath) return false;
  if (options.sandboxToken && data["sandboxToken"] !== options.sandboxToken) return false;
  return true;
}
...
export function handleRuntimeMessage(
  ...
  validation?: RuntimeMessageSessionValidationOptions,
): void {
  if (event.source !== frameWindow) return;
  const data = event.data as Record<string, unknown> | undefined;
  if (!data || data["source"] !== "hf-preview") return;
  if (!matchesSession(data, event, validation)) return;
