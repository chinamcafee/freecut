...
function execDataAttrPattern(html: string, attr: string, value: string): TagMatch | null {
  ...
  const tag = match?.[1];
  if (match?.index == null || !tag) return null;
  return { tag, start: match.index, end: match.index + tag.length };
}
...
function findTagByClass(html: string, target: PatchTarget): TagMatch | null {
  ...
  while ((match = pattern.exec(html)) !== null) {
    const tag = match[1];
    if (currentIndex === selectorIndex && match.index != null && tag) {
      return {
        tag,
        start: match.index,
...
export function findTagByTarget(html: string, target: PatchTarget): TagMatch | null {
  ...
  if (compositionId) {
    const result = execDataAttrPattern(html, "data-composition-id", compositionId);
    if (result) return result;
  }
...
function patchTextContent(html: string, elementId: string, value: string): string {
  ...
  if (!openingTag || !tagName) return html;
...
function patchHtmlAttribute(
  ...
  const tag = match[1];
  if (!tag) return html;
  return patchHtmlAttributeInTag(html, tag, attr, value);
}
